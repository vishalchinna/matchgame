...
class App extends Component {
  ...
  componentDidMount() {
    const s = imagesList.filter(each => each.category === 'FRUIT')
    this.setState({navList: s})
  ...
  arrangeThumbnails = tabId => {
    const newList = imagesList.filter(each => each.category === tabId)
    this.setState({navList: newList})
    this.setState({click: tabId})
  }
  ...
  onClickCheck = id => {
    ...
    const check = imagesList[matchimage].id === id
    if (check) {
      const k = Math.floor(Math.random() * 30)
      this.setState({matchimage: k})
  ...
  renderGameContent = () => {
    const {click, navList, matchimage} = this.state
    const image = imagesList[matchimage].imageUrl
    return (
      <div className="match-image">
        <img src={image} alt="match" className="match" />
        <ul className="activity-tabs">
          {tabsList.map(each => (
            <TabList
              ...
              tabs={each}
              click={click}
              arrangeThumbnails={this.arrangeThumbnails}
            />
          ))}
        </ul>
  ...
  renderResultContent = () => {
    ...
    return (
      <div className="result">
        <FinalResult score={score} restartGame={this.restartGame} />
      </div>
    )
  }

  render() {
    const {score, result, count} = this.state
    ...
    return (
      <div className="bg-container">
        ...
        {result ? this.renderGameContent() : this.renderResultContent()}
      </div>
    )
  }
}
